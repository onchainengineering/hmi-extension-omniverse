# from .extension import ViscaExtension, log
# from test import ViscaExtension, log
import pyaudio
from pydub import AudioSegment
import io


class ViscaAudioPlayer:
    ...
    def play(self):    
        if self.IsPlaying:
            return
        print("ViscaAudioPlayer - Started playing")
    ...
    def stream_callback(self, in_data, frame_count, time_info, status_flags):
        ...
        if self.AudioSegment and len(frames) < frame_count*self.AudioSegment.frame_width:
            print("ViscaAudioPlayer - Stopped playing")
...
if __name__ == '__main__':
    ...
    audio_player = ViscaAudioPlayer(None)
    ...
    def getGetResponseRequests(api_key: str, character_id: str, session_id: str = "") -> Generator[visca_service_msg.GetResponseRequest, None, None]:
        ...
        yield visca_service_msg.GetResponseRequest(
            get_response_config = get_response_config    
        )
        for i in range(0, int(RATE / CHUNK * RECORD_SECONDS)):
            data = stream.read(CHUNK)
            yield visca_service_msg.GetResponseRequest(
                get_response_data = visca_service_msg.GetResponseRequest.GetResponseData(
                    audio_data = data
                )
            )
        stream.stop_stream()
    ...
    #     audio_player = ViscaAudioPlayer(None)
    #     # data = stream.read(CHUNK)
    #     # _, data = scipy.io.wavfile.read("F:/Work/Visca/Tests/Welcome.wav")
    #     f = open("F:/Work/Visca/Tests/Welcome.wav", "rb")
    #     data = f.read()
    #     print(type(data))
